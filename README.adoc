...
The project **groupMembersFilter** is a Bonita Actor Filter for **Bonita {short-bonita-version}**  **Community** version written in `java` language.
...
[source,bash]
----
git clone https://your.github.com/groupMembersFilter.git
cd groupMembersFilter/
./mwnw package
----

The build should produce a jar archive under the `target/` folder named `groupMembersFilter-1.0-SNAPSHOT.jar`
